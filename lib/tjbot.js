...
class TJBot {
    ...
    */
    _setupCOMMON_ANODE_LED(){
      ...
      this._bluePin = new Gpio(12, {mode: Gpio.OUTPUT});
      this._greenPin = new Gpio(13, {mode: Gpio.OUTPUT});
      this._redPin = new Gpio(18, {mode: Gpio.OUTPUT});
    ...
    shine(color) {
        ...
        } else if(this._redPin){
          const rgb = `0x${c[1]}${c[2]}${c[3]}${c[4]}${c[5]}${c[6]}`;
          this._renderCommonAnodeLedColor(rgb);
        }
    }
    ...
    _renderCommonAnodeLedColor(hex){
      const rgb = this._convertHexToRgbColor(hex)
      winston.info("render color ===== ", rgb);
      this._redPin.pwmWrite(rgb[0] == null ? 255 : 255 - rgb[0]);
      this._.greenPin.pwmWrite(rgb[1] == null ? 255 : 255 -rgb[1]);
      this._.bluePin.pwmWrite(rgb[2] == null ? 255 : 255 -rgb[2]);
    }
