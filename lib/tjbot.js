...
class TJBot {
    ...
    async initialize(hardware) {
        ...
        hardware.forEach((device) => {
            ...
            case TJBot.HARDWARE.LED_COMMON_ANODE:
              this._setupCOMMON_ANODE_LED(this.configuration.shine.commonAnodeLedPin.redPin,
                this.configuration.shine.commonAnodeLedPin.greenPin,
                this.configuration.shine.commonAnodeLedPin.bluePin);
              break;
    ...
    * @private
    */
    _setupCOMMON_ANODE_LED(redPin, greenPin, bluePin) {
     ...
     * @private
     */
    _renderCommonAnodeLedColor(hexColor) {
      const rgb = this._convertHexToRgbColor(hexColor)
      if (this.configuration.shine.grbFormat) {
        this._commonAnodeLed.greenPin.pwmWrite(rgb[0] == null ? 255 : 255 - rgb[0]);
        this._commonAnodeLed.redPin.pwmWrite(rgb[1] == null ? 255 : 255 - rgb[1]);
      } else {
