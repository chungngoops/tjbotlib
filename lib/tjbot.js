...
class TJBot {
    ...
    */
    ...
    /**
    * Configure the LED hardware. The LED must be attached to the BCM 18 (PWM0) PIN.
    * @private
    */
    _setupLED() {
        winston.verbose(`initializing ${TJBot.HARDWARE.LED}`);
    ...
    * @private
    */
    _setupCOMMON_ANODE_LED(){
      winston.verbose(`initializing ${TJBot.HARDWARE.LED_COMMON_ANODE}`);

      this._ledCommonAnode['bluePin'] = new Gpio(12, {mode: Gpio.OUTPUT});
      this._ledCommonAnode['greenPin'] = new Gpio(13, {mode: Gpio.OUTPUT});
      this._ledCommonAnode['redPin'] = new Gpio(18, {mode: Gpio.OUTPUT});
    ...
    _assertCapability(capability) {
        ...
        case TJBot.CAPABILITIES.CONVERSE:
            if (!this.configuration.converse.assistantId) {
                throw new Error(
                    ...
                    + 'configuration.converse.assistantId parameter in the TJBot initialize() method.',
                );
            }
            if (!this._assistant) {
        ...
        case TJBot.CAPABILITIES.SHINE:
            if (!this._led || !this._ledCommonAnode) {
                throw new Error(
                    'TJBot is not configured with an LED. '
                    + 'Please check that you included the '
                    + `${TJBot.HARDWARE.LED} or ${TJBot.HARDWARE.LED_COMMON_ANODE} hardware in the TJBot initialize() method.`,
                );
            }
            break;
    ...
    async converse(message) {
        ...
        if (!this._assistantSessionId) {
            try {
                ...
                winston.error(`error creating session for ${TJBot.SERVICES.ASSISTANT} service. please check that configuration.converse.assistantId is defined.`);
                throw err;
            }
        }
        ...
        try {
            ...
            const responseText = body.output.text.length > 0 ? body.output.text[0] : '';
            const assistantResponse = {
                object: body,
                description: responseText,
            };
            winston.verbose(`received response from assistant: ${responseText}`);
     ...
     * @param  {function(string):void} callback Function to call when speech is detected.
     */
    listen(callback) {
        ...
        this._sttTextStream.on('data', (transcript) => {
    ...
    async recognizeObjectsInPhoto(filePath, classifierIds = []) {
        ...
        winston.debug(`sending image to the ${TJBot.SERVICES.VISUAL_RECOGNITION} service to recognize objects`);
        ...
        try {
            ...
            const result = body.images[0].classifiers[0].classes;
            return result;
        } catch (err) {
            winston.error(`the ${TJBot.SERVICES.VISUAL_RECOGNITION} service returned an error`, err);
            throw err;
    ...
    async takePhoto(filePath = '') {
        ...
        winston.debug(`capturing image at path: ${fp}`);
        ...
        try {
            ...
            const returnPath = path === '' ? (`${name}.${this._camera.getOptions().encoding}`) : (`${path}/${name}.${this._camera.getOptions().encoding}`);
            return returnPath;
        } catch (err) {
    ...
    shine(color) {
        ...
        if (this.configuration.shine.grbFormat) {
        ...
        } else if(this._ledCommonAnode){
          this._renderCommonAnodeLedColor(colors);
        }
    }
    ...
    _renderCommonAnodeLedColor(rgb){
      _ledCommonAnode.redPin.pwmWrite(255 - rgb[0]);
      _ledCommonAnode.greenPin.pwmWrite(255 -rgb[1]);
      _ledCommonAnode.bluePin.pwmWrite(255 -rgb[2]);
    }
    ...
    async speak(message) {
        ...
        if (this.configuration.speak.voice !== undefined) {
            ...
            if (!this._ttsVoices) {
                ...
                this._ttsVoices = await this._tts.listVoices();
            ...
            Object.values(this._ttsVoices).forEach((v) => {
                if (v.language === this.configuration.speak.language
                    && v.gender === this.configuration.robot.gender) {
                    voice = v.name;
                }
            });

            winston.verbose(`selected ${voice} as the ${this.configuraiton.robot.gender} voice for ${this.configuration.speak.language} `);
        }
        ...
        await this._tts.synthesize(params)
    ...
    async translate(text, sourceLanguage, targetLanguage) {
        ...
        try {
            translation = await this._languageTranslator.translate(params);
        } catch (err) {
    ...
    async identifyLanguage(text) {
        ...
        try {
            identifiedLanguages = await this._languageTranslator.identify(params);
        } catch (err) {
            winston.error(`the ${TJBot.SERVICES.LANGUAGE_TRANSLATOR} service returned an error`, err);
            throw err;
        }
    ...
    async isTranslatable(sourceLanguage, targetLanguage) {
        ...
        if (this._translations === undefined) {
            ...
            this._translations = await this._loadLanguageTranslations();
     ...
     * @async
     */
    async _loadLanguageTranslations() {
        let models;

        try {
            models = await this._languageTranslator.listModels({});
        } catch (err) {
            winston.error(`the ${TJBot.SERVICES.LANGUAGE_TRANSLATOR} service returned an error`, err);
            throw err;
        }
